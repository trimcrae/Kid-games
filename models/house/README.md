...
- [House from the street](previews/street_front.png)
- [Rear elevation from the back lawn](previews/rear_elevation.png)
...
photographs do. The `43 | Exterior roofs and cladding` collection is hidden
in the cutaway, plan and single-room views.

The yard trees are generated by a recursive limb algorithm in `yard.py`: a
curve of tapered branches, a dense mesh of small leaf quads for the render,
and a coarse stand-in of larger clusters that the browser export uses instead
(objects marked `export = False` are skipped by the exporter). The tall spruce beside the big front
tree is built the same way from whorls of drooping branches. Shrubs use
the same leaf clusters over an inner mass. Species and sizes are approximate;
no downloaded tree assets or image textures are used.
