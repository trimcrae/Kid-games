...
export async function createHouseLife(tour){
  ...
  for(const p of api.profiles()){
    ...
    bindButton(b,()=>{const switching=p.id!==engine.who();picked=true;savePosition();aftermath.stop();api.select(p.id);if(switching){arrivePop=true;lastWho=p.id;}restorePosition(p.id,switching);destination=null;sync(true);$('family-panel').hidden=true;tour.resume();});$('family-list').append(b);
  }
