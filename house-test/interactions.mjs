  ...
  for(const [key,colour,glass] of [['car','burgundy','#1e2b33'],['car2','black','#171d22']]){
    ...
    const groundAt=(px,pz)=>{
      ...
      for(const b of world.nearby(px,pz,.1)){
        ...
        // extra reach here, but that never makes an obstacle climbable.
        const reach=b.name==='Garage concrete slab'||b.name.startsWith('Asphalt apron graded to garage threshold walk strip ')?RAMP_REACH:STEP;
