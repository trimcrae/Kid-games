...
(async()=>{
  ...
  try{
    ...
    await mobile.locator('#rooms-button').tap();await mobile.locator('#rooms').waitFor({state:'visible'});await mobile.getByRole('button',{name:'Jump to Front entry',exact:true}).tap();await mobile.waitForTimeout(1100);
    ...
    await mobile.locator('#rooms-button').tap();await mobile.locator('#rooms').waitFor({state:'visible'});await mobile.getByRole('button',{name:'Jump to Back yard',exact:true}).tap();await mobile.waitForTimeout(1100);await mobile.locator('[data-activity="farm"]').tap();
