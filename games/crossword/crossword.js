/* ===========================================================
   ...
   jump to it. Full keyboard play too: letters type, arrows move
   (and turn the cursor), Tab hops clue to clue, Enter flips
   Across/Down, Backspace rubs out. Check shows which letters to
   fix (kindly, and the marks fade); Reveal needs two taps.

   Hints TEACH before they help: the first ask explains a spelling
   rule, a fact or what the word means; the second gives the length
   and first letter; only the third hands over a letter. Solving with
   no letter-hints and no Reveal earns a ⭐.

   Progress, per-puzzle best times and a day streak are saved in
   localStorage per puzzle INDEX, so puzzles are only ever appended.

   Teaches: spelling, vocabulary and clue-reading. Themes run from
   easy (colours, farm animals) through medium and tricky up to a
   real grown-up tier, and include safe pop culture (Minecraft,
   Bluey, Frozen, favourite books, taekwondo, animals).
...
(function () {
  ...
  function slot(i) {
    const k = "p" + i;
    if (!saved[k] || typeof saved[k] !== "object") saved[k] = { letters: null, solved: false };
    const s = saved[k];
    if (typeof s.secs !== "number") s.secs = 0;        // time spent on this puzzle
    if (typeof s.best !== "number") s.best = 0;        // best clean solve, seconds (0 = none)
    if (typeof s.letterHints !== "number") s.letterHints = 0;
    if (typeof s.revealed !== "boolean") s.revealed = false;
    return s;
  }
  function meta() {
    if (!saved.meta || typeof saved.meta !== "object") saved.meta = {};
    const m = saved.meta;
    if (typeof m.streak !== "number") m.streak = 0;
    if (typeof m.bestStreak !== "number") m.bestStreak = 0;
    if (typeof m.lastDay !== "string") m.lastDay = "";
    if (typeof m.timer !== "boolean") m.timer = false;
    if (typeof m.filter !== "string") m.filter = "all";
    return m;
  }
  /* a "days in a row" streak — the reason to come back tomorrow */
  function dayStamp(d) {
    const p = (n) => (n < 10 ? "0" : "") + n;
  ...
  function touchStreak() {
    ...
    m.streak = m.lastDay === dayStamp(y) ? m.streak + 1 : 1;
    m.lastDay = today;
  ...
  const el = {
    ...
    levels: $("level-chips"), surprise: $("surprise-btn"),
    timer: $("timer"), timerBtn: $("timer-btn"), streak: $("streak-line"),
  };
  ...
  let revealTimer = 0;
  let badTimer = 0;
  let hintStage = {};       // "num-dir" -> how many hints this clue has already given
  let secs = 0, tickId = 0; // play clock for the current puzzle
  ...
  const LEVELS = [
    ...
    { id: "grown-up", label: "🧠 Grown-up" },
  ];
  const LEVEL_TAG = { easy: "🌱 Easy", medium: "🙂 Medium", tricky: "🔥 Tricky", "grown-up": "🧠 Grown-up" };
  ...
  function renderLevelChips() {
    ...
    el.levels.innerHTML = "";
    LEVELS.forEach((lv) => {
      const n = lv.id === "all" ? PUZZLES.length : PUZZLES.filter((p) => p.level === lv.id).length;
      if (!n) return;
      const b = document.createElement("button");
      b.type = "button";
      b.className = "lvl-chip" + (meta().filter === lv.id ? " on" : "");
      b.textContent = lv.label + " (" + n + ")";
      b.setAttribute("aria-pressed", meta().filter === lv.id ? "true" : "false");
      b.addEventListener("click", () => { meta().filter = lv.id; save(); renderPuzzles(); });
  ...
  function renderPuzzles() {
    ...
    PUZZLES.forEach((p, i) => {
      const st = slot(i);
      if (st.solved) { wins++; if (!st.revealed && !st.letterHints) stars++; }
    });
    PUZZLES.forEach((p, i) => {
      if (filter !== "all" && p.level !== filter) return;
      const st = slot(i);
      const done = st.solved;
      const clean = done && !st.revealed && !st.letterHints;
      const started = !done && Array.isArray(st.letters) &&
        st.letters.some((row) => row && row.some((x) => x));
      const card = document.createElement("button");
      card.type = "button";
      card.className = "puz-card" + (done ? " solved" : "") + (started ? " started" : "");
      const status = done
        ? (clean ? "⭐ Solved on your own!" : "✓ Solved")
        : started ? "▸ Keep going…" : p.blurb;
      const time = done && st.best ? '<span class="pz-time">🏅 ' + fmtTime(st.best) + "</span>" : "";
      card.innerHTML =
        '<span class="pz-emoji" aria-hidden="true">' + p.emoji + "</span>" +
        "<h3>" + p.name + "</h3>" +
        '<span class="pz-blurb">' + status + "</span>" +
        '<span class="pz-lvl">' + (LEVEL_TAG[p.level] || "") + "</span>" + time;
      card.setAttribute("aria-label",
        p.name + ", " + (LEVEL_TAG[p.level] || "").replace(/[^A-Za-z-]/g, "") + ". " +
        (done ? "Solved." : p.blurb));
    ...
    if (el.streak) {
      const m = meta();
      const bits = [];
      if (m.streak > 1) bits.push("🔥 " + m.streak + " days in a row");
      else if (m.streak === 1) bits.push("🔥 Day 1 of a new streak");
      if (stars) bits.push("⭐ " + stars + " solved with no help");
      if (m.bestStreak > m.streak) bits.push("Best streak: " + m.bestStreak + " days");
      el.streak.textContent = bits.join("  ·  ");
    }
    renderLevelChips();
  }
  ...
  function startPuzzle(i) {
    ...
    if (el.title) el.title.setAttribute("aria-label", puzzle.name + " crossword");
    ...
    const st = slot(i);
    user = puzzle.cells.map((row) => row.map((ch) => (ch === null ? null : "")));
    if (Array.isArray(st.letters)) {
      for (let r = 0; r < puzzle.rows; r++)
        for (let c = 0; c < puzzle.cols; c++)
          if (user[r] && user[r][c] === "" && st.letters[r] && typeof st.letters[r][c] === "string")
            user[r][c] = st.letters[r][c].toUpperCase().replace(/[^A-Z]/g, "").slice(0, 1);
    }

    // show the board FIRST — sizeGrid() measures the layout, and a hidden
    // section measures 0, which used to leave the cells the wrong size.
    show("play");
    buildGrid();
    buildClues();
    markDoneClues(true); // seed quietly so restored words don't re-ding
    sizeGrid();
    // focus the first entry that still has an empty square
    const first = nextIncompleteEntry(null) ||
  ...
  /* the grid has to fit the phone, whatever the puzzle's width — so the
     cell size is measured, not guessed (a 14-column grid at a fixed
  ...
  function sizeGrid() {
    ...
    // the very widest grids get a slightly smaller floor so they still fit a
    // phone; anything narrower keeps a comfortable 26px minimum tap target
    cell = Math.max(puzzle.cols >= 13 ? 24 : 26, Math.min(40, cell));
    el.grid.style.gap = gap + "px";
    el.grid.style.gridTemplateColumns = "repeat(" + puzzle.cols + ", " + cell + "px)";
    el.grid.style.gridAutoRows = cell + "px";
    el.grid.style.fontSize = Math.round(cell * 0.56) + "px";
    el.grid.style.setProperty("--num-size", Math.max(8, Math.round(cell * 0.3)) + "px");
  }

  function cellLabel(r, c) {
    const e = entriesAt(r, c);
    const bits = [];
    if (e.across) bits.push(e.across.num + " across, letter " + (c - e.across.col + 1) + " of " + e.across.answer.length);
    if (e.down) bits.push(e.down.num + " down, letter " + (r - e.down.row + 1) + " of " + e.down.answer.length);
    return bits.join(", ") || "row " + (r + 1) + " column " + (c + 1);
  }

  function buildGrid() {
    el.grid.innerHTML = "";
    el.grid.setAttribute("role", "group");
    el.grid.setAttribute("aria-label", puzzle.name + " crossword grid, " + puzzle.rows + " rows by " + puzzle.cols + " columns");
    inputs = [];
    for (let r = 0; r < puzzle.rows; r++) {
      inputs[r] = [];
      for (let c = 0; c < puzzle.cols; c++) {
  ...
  function buildClues() {
    ...
    puzzle.entries.forEach((e) => {
      ...
      b.type = "button";
      b.className = "clue-li";
      b.dataset.num = e.num; b.dataset.dir = e.dir;
      b.innerHTML = '<span class="cnum">' + e.num + ".</span><span class=\"ctext\">" + e.clue + "</span>";
      b.setAttribute("aria-label", e.num + " " + e.dir + ": " + e.clue);
      b.addEventListener("click", () => selectEntry(e));
      li.appendChild(b);
  ...
  function curEntry() { return curCell ? entriesAt(curCell.r, curCell.c)[curDir] || null : null; }

  function selectCell(r, c, toggle) {
    const e = entriesAt(r, c);
    let dir = curDir;
    if (toggle && curCell && curCell.r === r && curCell.c === c && e.across && e.down) {
      dir = curDir === "across" ? "down" : "across";
    }
  ...
  function selectEntry(e, quiet) {
    curDir = e.dir;
    // jump to the first empty cell of the entry, else its first cell
  ...
  function highlight() {
    ...
    document.querySelectorAll(".clue-li").forEach((b) => { b.classList.remove("active"); b.removeAttribute("aria-current"); });
    if (!curCell || !inputs[curCell.r] || !inputs[curCell.r][curCell.c]) { updateClueBar(null); return; }
    // never leave the cursor pointing at a direction this cell has no word in
    if (!entriesAt(curCell.r, curCell.c)[curDir]) {
      curDir = entriesAt(curCell.r, curCell.c).across ? "across" : "down";
    }
    const e = curEntry();
    if (e) {
      entryCells(e).forEach((cell) => {
        const inp = inputs[cell.r] && inputs[cell.r][cell.c];
        if (inp && inp.parentElement) inp.parentElement.classList.add("active");
      });
      const list = e.dir === "across" ? el.across : el.down;
      list.querySelectorAll(".clue-li").forEach((b) => {
        if (+b.dataset.num === e.num && b.dataset.dir === e.dir) {
    ...
    inputs[curCell.r][curCell.c].parentElement.classList.add("cursor");
  }
  ...
  function updateClueBar(e) {
    ...
    el.curClueText.innerHTML =
      ...
      ' <span class="len">(' + e.answer.length + " letters)</span>";
  }
  ...
  // the next clue (wrapping) that still has an empty square in it
  function nextIncompleteEntry(from) {
    const list = puzzle.entries;
    const start = from ? list.indexOf(from) : -1;
    for (let k = 1; k <= list.length; k++) {
      const e = list[(start + k + list.length) % list.length];
      if (entryCells(e).some((cell) => !user[cell.r][cell.c])) return e;
    }
    return null;
  }
  ...
  function nextCellInEntry(dir, forward) {
    ...
    return cells[idx + (forward ? 1 : -1)] || null;
  }

  // walk over black squares to the next real cell in a direction
  function stepOver(r, c, dr, dc) {
    let rr = r + dr, cc = c + dc;
    while (rr >= 0 && rr < puzzle.rows && cc >= 0 && cc < puzzle.cols) {
      if (puzzle.cells[rr][cc] !== null) return { r: rr, c: cc };
      rr += dr; cc += dc;
    }
    return null;
  }
  ...
  // after typing: next empty square in this word, else the next square,
  // else hop to the next clue that still needs letters
  function advance() {
    const e = curEntry();
    if (!e) return;
    const cells = entryCells(e);
    ...
    for (let k = idx + 1; k < cells.length; k++) {
      if (!user[cells[k].r][cells[k].c]) return moveTo(cells[k]);
    }
    if (idx + 1 < cells.length) return moveTo(cells[idx + 1]);
    const nx = nextIncompleteEntry(e);
    if (nx && nx !== e) selectEntry(nx);
  }
  ...
  function onKeyDown(e, r, c) {
    ...
    if (k.length === 1 && /[a-zA-Z]/.test(k) && !e.ctrlKey && !e.metaKey && !e.altKey) {
      // handle letters ourselves so typing over a filled square always works
      e.preventDefault();
      setLetter(r, c, k.toUpperCase());
      advance();
      checkSolved();
      return;
    }
    if (k === "Backspace") {
      e.preventDefault();
      if (user[r][c]) setLetter(r, c, "");
      else {
        const prev = nextCellInEntry(curDir, false);
        if (prev) { setLetter(prev.r, prev.c, ""); moveTo(prev); }
      }
      markDoneClues(true);
      return;
    }
    if (k === "Delete") { e.preventDefault(); setLetter(r, c, ""); markDoneClues(true); return; }
    if (k === "ArrowRight" || k === "ArrowLeft" || k === "ArrowUp" || k === "ArrowDown") {
      e.preventDefault();
      const axis = (k === "ArrowRight" || k === "ArrowLeft") ? "across" : "down";
      const dr = k === "ArrowDown" ? 1 : k === "ArrowUp" ? -1 : 0;
      const dc = k === "ArrowRight" ? 1 : k === "ArrowLeft" ? -1 : 0;
      // first press of a cross-axis arrow just turns the cursor, like a real crossword
      if (curDir !== axis && entriesAt(r, c)[axis]) { curDir = axis; highlight(); return; }
      const nxt = stepOver(r, c, dr, dc);
      if (nxt) {
        if (entriesAt(nxt.r, nxt.c)[axis]) curDir = axis;
        moveTo(nxt);
      }
      return;
    }
    if (k === "Tab") { e.preventDefault(); stepEntry(e.shiftKey ? -1 : 1); return; }
    if (k === "Enter" || k === " ") { e.preventDefault(); selectCell(r, c, true); return; }
    if (k === "Home" || k === "End") {
      e.preventDefault();
      const en = curEntry();
      if (en) { const cells = entryCells(en); moveTo(k === "Home" ? cells[0] : cells[cells.length - 1]); }
    }
  }
  ...
  function markDoneClues(silent) {
    document.querySelectorAll(".clue-li").forEach((b) => {
      const e = puzzle.entries.find((x) => x.num === +b.dataset.num && x.dir === b.dataset.dir);
  ...
  function doCheck() {
    ...
    if (filled === 0) flash("Type some answers, then press Check! ✏️", "var(--purple)");
      ...
      flash("Every letter so far is right — " + blanks + " square" + (blanks === 1 ? "" : "s") + " to go! 👍", "var(--green)");
      window.SFX && SFX.good();
    } else {
      flash(wrong === 1
        ? "So close! 1 letter to swap — it's in the pink box 🩷"
        : "Nearly! " + wrong + " letters to swap — they're in the pink boxes 🩷", "var(--pink)");
      window.SFX && SFX.nope();
    }
    // the pink marks fade on their own, so nothing feels like a permanent X
    badTimer = setTimeout(clearBadMarks, 6000);
  }

  /* Hints that TEACH: the first ask for a clue explains something about
     the word (a spelling rule, a fact, what it means). Only if the kid
     asks again for the same clue does a real letter appear. */
  function doHint() {
    if (!puzzle || solvedNow) return;
    const e = curEntry();
    if (!e) { flash("Tap a square or a clue first, then I'll help. 💡", "var(--purple)"); return; }
    const key = e.num + "-" + e.dir;
    const stage = hintStage[key] || 0;
    const cells = entryCells(e);
    const done = cells.every((x) => user[x.r][x.c] === puzzle.cells[x.r][x.c]);
    if (done) { flash("That word's already perfect — pick another clue! 💡", "var(--purple)"); return; }

    if (stage === 0 && e.teach) {
      hintStage[key] = 1;
      flash("💡 " + e.teach, "var(--blue)");
      window.SFX && SFX.good();
      return;
    }
    if (stage <= 1) {
      hintStage[key] = 2;
      flash("💡 " + e.answer.length + " letters, and it starts with " + e.answer[0] +
        ". Ask again for a free letter!", "var(--blue)");
      window.SFX && SFX.good();
      return;
    }
    // third ask (and after): fill one letter of this word
    let cell = cells.find((x) => user[x.r][x.c] !== puzzle.cells[x.r][x.c]);
    if (!cell) { flash("That word's already perfect — pick another clue! 💡", "var(--purple)"); return; }
    slot(pi).letterHints++;
    setLetter(cell.r, cell.c, puzzle.cells[cell.r][cell.c]);
    const w = inputs[cell.r][cell.c].parentElement;
  ...
  function doReveal() {
    if (!revealArmed || Date.now() - revealArmed > 5000) {
  ...
  function stopClock() { if (tickId) { clearInterval(tickId); tickId = 0; } if (puzzle) { slot(pi).secs = secs; save(); } }
  function updateTimerUI() {
    if (!el.timer) return;
    const on = meta().timer;
    el.timer.textContent = "⏱ " + fmtTime(secs);
    el.timer.classList.toggle("hidden", !on);
    if (el.timerBtn) {
      el.timerBtn.setAttribute("aria-pressed", on ? "true" : "false");
      el.timerBtn.textContent = on ? "⏱ Hide timer" : "⏱ Show timer";
    }
  }
  ...
  el.quit.addEventListener("click", leavePlay);
  el.next.addEventListener("click", () => {
    const j = nextUnsolved();
    if (j === -1) leavePlay();
    else startPuzzle(j);
  });
  if (el.timerBtn) el.timerBtn.addEventListener("click", () => {
    meta().timer = !meta().timer; save(); updateTimerUI();
  });
  if (el.surprise) el.surprise.addEventListener("click", () => {
    const pool = PUZZLES.map((p, i) => i).filter((i) => !slot(i).solved);
    const list = pool.length ? pool : PUZZLES.map((p, i) => i);
    startPuzzle(list[Math.floor(Math.random() * list.length)]);
  });

  window.addEventListener("resize", sizeGrid);
  window.addEventListener("orientationchange", () => setTimeout(sizeGrid, 120));
  window.addEventListener("pagehide", stopClock);
  document.addEventListener("visibilitychange", () => {
    if (document.hidden) stopClock();
    else if (puzzle && !el.play.classList.contains("hidden") && !solvedNow) startClock(secs);
  });
