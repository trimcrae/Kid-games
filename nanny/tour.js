...
document.querySelector('#print-guide').innerHTML = Array.from({length:Math.ceil(stops.length/2)},(_,page)=>`<section class="print-page"><h1>Nearby blue-gray houses</h1><p>Within about 2 miles of Renaissance Academy · September 12, 2026</p>${stops.slice(page*2,page*2+2).map(s=>`<article class="print-house"><h2>${s.address}</h2><p>Rochester, NY ${s.zip} · about ${s.miles.toFixed(1)} miles from the school</p><div class="print-house-body"><figure><img src="${thumbnail(s)}" width="900" height="600" alt="${s.title} at ${s.address}"><figcaption>Google Street View · ${s.date} · © Google</figcaption></figure><div><p><strong>Color guess:</strong></p>${colorGuess(s)}<p class="print-footnote">${s.caveat}</p></div></div><p class="print-notes">Notes: __________________________________________________________________</p></article>`).join('')}<p class="print-footnote">This contains every verified blue-gray house found within the stated boundary. Colors are visual estimates, not confirmed paint matches. Photos may be older than the current finish. Live list: https://trimcrae.github.io/Kid-games/nanny/</p></section>`).join('');
